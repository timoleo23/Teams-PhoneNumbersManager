...
function Get-CsCallQueueModern {
    [CmdletBinding()]
    param(
    ...
    process {
        $result = Microsoft.Teams.ConfigAPI.Cmdlets.internal\Get-CsCallQueue @PSBoundParameters
        Write-AdminServiceDiagnostic($result.Diagnostic)

        if (${Identity} -ne '') {
            $callQueue = [Microsoft.Rtc.Management.Hosted.CallQueue.Models.CallQueue]::new()
            $callQueue.ParseFrom($result.CallQueue)
        } else {
            $callQueues = @()
            foreach ($model in $result.CallQueue) {
                $callQueue = [Microsoft.Rtc.Management.Hosted.CallQueue.Models.CallQueue]::new()
                if ($ExcludeContent) {
                    $callQueues += $callQueue.ParseFrom($model, $true)
                } else {
                    $callQueues += $callQueue.ParseFrom($model)
                }
            }
            $callQueues
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCGFGIh3GWINfuP
# GYO+2rj0BjxMHFUiGW0CQbNn4fGGyqCCDXMwggXxMIID2aADAgECAhMzAAACGYwK
...
# CisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIARC/E3WH+AnnhyCAqpuX4yWePeV
# nZ1sRYz9k/vzPqwHMDoGCisGAQQBgjcCAQwxLDAqoAyACgBTAGsAeQBwAGWhGoAY
# aHR0cDovL3d3dy5taWNyb3NvZnQuY29tMA0GCSqGSIb3DQEBAQUABIIBAHBfsmTu
# GLcASGNPO0pX74BCXYI/XVLxjQLvt4gSRj4NuqcqvOlsIQnLZ2zfdMfuqroxnegG
# rKj7ck/XoUck0Oo27Wp2k3QYaV361pNZfN00c7/RPsrxy18mk0Nl6tGU/GC5aGwu
# tf+Zj5N6dz1QzrOlIwKK+Te8onYqq41zPCDHZvQrUB+O0KdQ+HWFOmln5z8Jsfkq
# NP5Ho6g8HdEkAIJ1XavmcNzbcWo6OE+Gjo3/g5hFJGRwnqhjEDjIBUl/Wvid9CwC
# p2oc0Sc4oI0IaiSFIwSpHFYzv3A2dxD0iT4vLJe1CAzpzKZK/tCeDsENm+EhekDr
# QoQv/A6fPkVeik2hghcZMIIXFQYKKwYBBAGCNwMDATGCFwUwghcBBgkqhkiG9w0B
# BwKgghbyMIIW7gIBAzEPMA0GCWCGSAFlAwQCAQUAMIIBWQYLKoZIhvcNAQkQAQSg
# ggFIBIIBRDCCAUACAQEGCisGAQQBhFkKAwEwMTANBglghkgBZQMEAgEFAAQgy6eK
# uLxygCafCE1xRv9WO0tv1j+rct7/nzDoaOgK+okCBmGVXfZEHBgTMjAyMTExMzAw
# NjQ5MzIuMTgxWjAEgAIB9KCB2KSB1TCB0jELMAkGA1UEBhMCVVMxEzARBgNVBAgT
...
# 9w0BCRABBDAvBgkqhkiG9w0BCQQxIgQgAAzNHGtZzWuqpmtu4FceeS+DMfo9alzo
# LnwCrS8IJJkwgfoGCyqGSIb3DQEJEAIvMYHqMIHnMIHkMIG9BCD0veCrdd9Kvn7z
...
# VFpeZth1IolYcPj8Oie0+ggsEzoZMA0GCSqGSIb3DQEBCwUABIICAHYtYpqhn4SB
# Uy8fjHSI+9toOBzQpuPYw4SSYavrEO+xIXR4xe/Ca26X6RQx5T66EHzW7zXePtOQ
# ZohXLNo0V07LiQlpWYg48norAN5hACj4UvVfw7TbpNZ/0WfsT+SLB4wjTwrKKfjl
# BXWis1UfdvBIVg3CTcRS94mlwRsGRsja/raDSXZpYqhj+bzkrUnt9yMEEj1c4fZs
# Y+dZPx3e0SR+CA8zuFLE5yLScarNDugEghDzaK02F1dQ60/s7FKY77liR1y7EmbY
# OLmvohUAfTRyi90x0Glh+Rr6iqMRjYZvO/5H5z1E8IZ+grihDgEm7/n4MQeRZWu9
# vSqT7iBqBYKmYDRLcYj4HCKbyF6S8H4clvZD7AjA+7dNGsQ9wolRGY5s8CiDHeGe
# 2cxyj2H/dBjd2JhQ+sLGzz7TKlkVKnevNL5IZWHniMFHn4ZHFT4i2Wp4JLEh+ww+
# H/Vtrlxr8hcH690PJbPKoPnWOgDAFefIqUqyAbKTHnBjQu1scvFcXV69ugid37iT
# ezBGabEt/pFKPle8uag9MMV6K3tKYBkGdzX0ea7jYSUCfmuZLDpbDUTYTwldJZxh
# sFtdm2gsCRnjVTaS89KTzjYP2b3RfV7pYUYn/psQar/efVLzbla0pZdSWjzS1+z+
# FSa3D/8lPqeGyw8jNYGX9T0vi42gn/LR
