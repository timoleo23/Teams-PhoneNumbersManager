...
function New-CsOnlineScheduleModern {
    ...
    process {
        ...
        if ($WeeklyRecurrentSchedule) {
            if ($MondayHours -ne $null -and $MondayHours.Length -gt 0) { $params['WeeklyRecurrentScheduleMondayHour'] = $MondayHours }
            if ($TuesdayHours -ne $null -and $TuesdayHours.Length -gt 0) { $params['WeeklyRecurrentScheduleTuesdayHour'] = $TuesdayHours }
            if ($WednesdayHours -ne $null -and $WednesdayHours.Length -gt 0) { $params['WeeklyRecurrentScheduleWednesdayHour'] = $WednesdayHours }
            if ($ThursdayHours -ne $null -and $ThursdayHours.Length -gt 0) { $params['WeeklyRecurrentScheduleThursdayHour'] = $ThursdayHours }
            if ($FridayHours -ne $null -and $FridayHours.Length -gt 0) { $params['WeeklyRecurrentScheduleFridayHour'] = $FridayHours }
            if ($SaturdayHours -ne $null -and $SaturdayHours.Length -gt 0) { $params['WeeklyRecurrentScheduleSaturdayHour'] = $SaturdayHours }
            if ($SundayHours -ne $null -and $SundayHours.Length -gt 0) { $params['WeeklyRecurrentScheduleSundayHour'] = $SundayHours }
            if ($Complement) { $params['WeeklyRecurrentScheduleIsComplemented'] = $true }
        }

        $result = Microsoft.Teams.ConfigAPI.Cmdlets.internal\New-CsOnlineSchedule @params
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC0uf/gaqQLE1O+
# dveyBH70cSJQflfU+17orP1GZ7Y7sKCCDXMwggXxMIID2aADAgECAhMzAAACGYwK
...
# CisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIEseCSLFNlbyHELdcHncjvVbFIi6
# erky+GuxbZefpVRmMDoGCisGAQQBgjcCAQwxLDAqoAyACgBTAGsAeQBwAGWhGoAY
# aHR0cDovL3d3dy5taWNyb3NvZnQuY29tMA0GCSqGSIb3DQEBAQUABIIBAE+6hT0T
# ZSn9HOth3ddINsIPFBVXH3m04W81/Xrkc+rs2Ts85CozRwAvz21wvLotOecQUhHO
# aXxEmFigzPbYXaDzYNpTrN5NRJmCbCERnON897Tlx4WcueqYiItAjsZnAMnpuAck
# SZiBqZX7IEKY9U0X/3EMhO+QXCMjkzsPzoKf1ghdI17hECGHOGY9NNLNG9jAHPra
# JTFtIx9mSFtOMaBJKZmk9Qza01EtJ1YpsLIbWtsER0/WOJe3ox2u2VdUgsyoQOS/
# sl3DrJBy6JHDDhfjdwtGlykNmgNYClyniazxbzeclpVGPoQa1JuH89s5rqDEPQFt
# VBy9FbtDLGysyB+hghLkMIIS4AYKKwYBBAGCNwMDATGCEtAwghLMBgkqhkiG9w0B
# BwKgghK9MIISuQIBAzEPMA0GCWCGSAFlAwQCAQUAMIIBUAYLKoZIhvcNAQkQAQSg
# ggE/BIIBOzCCATcCAQEGCisGAQQBhFkKAwEwMTANBglghkgBZQMEAgEFAAQgxUKn
# 5u8itoKUvwpSZDPaGEgHqbsSVkO5XpWv9r7YvIUCBmFDqUd4LxgSMjAyMTA5MzAx
# NjMzMDMuMzdaMASAAgH0oIHQpIHNMIHKMQswCQYDVQQGEwJVUzETMBEGA1UECBMK
...
# AQQwLwYJKoZIhvcNAQkEMSIEIL13EMRE8YZGhI1BB54FmIfTCvjE6Io66Qh3C0zF
# ivxMMIH6BgsqhkiG9w0BCRACLzGB6jCB5zCB5DCBvQQgAGcmEPaCWKTAxnIhbRhy
...
# 3UqF23gteoNryeMlREX4mTANBgkqhkiG9w0BAQsFAASCAQA5DIr+o0FUzFhnFrNP
# GzUgA0v8JCr8eb7g37f5FYtBGz8tjKgHyJkCDPxjYll9V2D505N7cvWqQFHoFu7H
# ceh7wmXOTlkaY87kT8U7D/xj9G205qH33txjJ0uZ18eotGKKNMzJeIMHjk1+dlwZ
# R7BV6DDYgE7EhxYnj0Co2atMAd3hsGvluXLR0AoNzJpetrcJeCgX2sLAsq0IVyXV
# 02qhic0Do/hTnn+T+re+aJDgyaV6lLyAPv3EuUNfiAnznjcZDla/yqGMIFdMe/aX
# x+VPFeye2N0l2bM/WLT5bBgLAI+Jgsak8y0TeCED5FeEeucc43AkcFir3fVlqse8
# aYO7
