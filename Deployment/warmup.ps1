...
function checkStatus($jobStatus) {
    ...
    foreach ($item in $jobStatus) {
        if ($item.StatusCode -ne 200) {$result = "NOK"}
    }
    return $result
}
...
Do
{
    Write-Host "Function warm-up started at" $(Get-Date) "- Retry #"$retries
    $job = generateConfig $hostname $code $upn $workers $retries | ForEach-Object -ThrottleLimit $workers -Parallel { 
        ...
        $Resp = New-Object -TypeName psobject -Property @{Duration= [Math]::Round($duration,2); StatusCode= $Result.StatusCode; StatusDescription= If($duration -gt $timeout) {"Request timed out ($timeout sec)"} Else {$Result.StatusDescription};TriggerTime= (Get-Date -DisplayHint Time)}
        return $Resp
    } -AsJob
    $jobresults = $job | Wait-Job | Receive-Job
    $job | Remove-Job
    $retries +=1
}
until( (checkStatus($jobresults) -eq "OK") -OR ($retries -ge $maxRetry))
